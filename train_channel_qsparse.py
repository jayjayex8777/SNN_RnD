"""
Phase 4: Channel-Variant INT8 Static Quantization for Sparse SNN
...
LOG_FILE = "train_channel_qsparse.log"
...
# ========================= Weight Loading =========================
def load_sparse_weights(variant_name, c1, c2, T, target_rate):
...
def main():
    ...
    total_configs = len(T_VALUES) * len(VARIANT_CHANNELS) * len(TARGET_RATES)

    print("=" * 80, flush=True)
    print("  Phase 4: Channel-Variant INT8 Static Quantization", flush=True)
    print("=" * 80, flush=True)
    print(f"  T values:      {T_VALUES}", flush=True)
    print(f"  Kernel size:   {KERNEL_SIZE} (fixed)", flush=True)
    print(f"  Variants:      {list(VARIANT_CHANNELS.keys())}", flush=True)
    ...
    for T in T_VALUES:
        ...
        for vname, (c1, c2) in VARIANT_CHANNELS.items():
            for target_rate in TARGET_RATES:
                ...
                print(f"\n[{idx:3d}/{total_configs}] {tag} (c1={c1},c2={c2})  "
                      f"(elapsed: {elapsed:.0f}s, ETA: {eta:.0f}s)", flush=True)

                model, pt_path = load_sparse_weights(vname, c1, c2, T, target_rate)
                if model is None:
    ...
    # ========================= Save Results =========================
    result_path = os.path.join(RESULT_DIR, "channel_variant_qsparse_results.json")
    ...
    print(f"\n\n{'='*80}", flush=True)
    print(f"  SUMMARY: Channel-Variant INT8 Quantization Complete", flush=True)
    ...
    if all_results:
        print(f"\n{'Variant':<10} {'Ch':>10} {'T':>3} {'FR%':>5} {'FP32':>8} {'INT8':>8} "
        ...
        for r in all_results:
            ...
            print(f"{r['variant']:<10} {ch_str:>10} {r['T']:>3} {fr_pct:>5} "
    ...
    if all_results:
        ...
        for r in sorted_by_acc[:5]:
            fr_pct = int(r['target_rate'] * 100)
            ch_str = f"({r['channels'][0]},{r['channels'][1]})"
            print(f"    {r['variant']} {ch_str} T={r['T']} fr={fr_pct}%  "
