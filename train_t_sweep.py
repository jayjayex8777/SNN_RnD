"""T-step sweep: Train SNN teacher + student with T=5, 10, 15 (T=20 already done).
Uses the same kernel sizes as kernel_sweep (k=3,5,7,9,11) mapped to variant names.
Exports TorchScript Lite models for mobile inference.
"""
import json
import os
...
def main():
    ...
    print(f"T values to train: {T_VALUES}")
    ...
    for T in T_VALUES:
        ...
        # Create dataset with this T value
        ...
        for ks in KERNEL_SIZES:
            ...
            # --- Teacher ---
            print(f"\n--- Teacher {vname} (k={ks}, T={T}) ---")
            teacher = TeacherSNN1d(C1, C2, kernel_size=ks, T=T).to(DEVICE)
            ...
            teacher_result = {
                "T": T, "variant": vname, "kernel_size": ks, "type": "teacher",
            ...
            # --- Student KD ---
            print(f"\n--- Student {vname} (k={ks}, T={T}) ---")
            teacher.eval()
            student = StudentSNN1d(C1, C2, kernel_size=ks, T=T).to(DEVICE)
            ...
            student_result = {
                "T": T, "variant": vname, "kernel_size": ks, "type": "student",
    ...
    # Save results
    result_path = os.path.join(RESULT_DIR, "t_sweep_results.json")
    ...
    for r in all_results:
        print(f"  T={r['T']} {r['type']:8s} {r['variant']:8s} (k={r['kernel_size']}): "
